...
def task_create_data(depends_on, produces):
    ...
    starts = policy_data[["start_date", "scale", "flag", "recorded_flag", 
                          "maximum", "row"]].rename(columns={"start_date": "date"})
    ends = policy_data[["end_date", "scale", "flag", "recorded_flag", 
                         "maximum", "row"]].rename(columns={"end_date": "date"})
    ...
    covid_policy.columns = ["_".join(col) for col in stringency_index.columns]
    """Fill missing values
    """
